...
test("bareJid strips resource and lowercases", () => {
    assert.equal(
        bareJid("USER@example.org/Conversations"),
        "user@example.org",
    );
    assert.equal(bareJid("user@example.org"), "user@example.org");
});

test("inboundFrom rejects forged received carbons", () => {
    const self = "pi@example.org";
    const peer = "user@example.org/phone";
    assert.equal(
        inboundFrom({
            outerFrom: self,
            innerFrom: peer,
...
test("allowedFrom matches bare JID only", () => {
    assert.equal(
        allowedFrom("user@example.org/phone", "user@example.org"),
        true,
    );
    assert.equal(
        allowedFrom("other@example.org/phone", "user@example.org"),
        false,
    );
});
